...
class Solution:
    def isValidSudoku(self, board: List[List[str]]) -> bool:
        ...
        square_dict: defaultdict[int, set] = defaultdict(set) #Each key 0-8 represents the square number and the values present there up to that point
        for row in range (0, 9):
            for col in range(0, 9):
                ...
                square_index : int = self.square_index_calculator(row, col)
                if current_character in square_dict[square_index]: return False
                else: square_dict[square_index].add(current_character)
    ...
    def square_index_calculator(self, row : int, col : int) -> int:
        result : int = 0
        if col <= 2: result += 0
        elif col <= 5: result += 1
        elif col <= 8: result += 2
